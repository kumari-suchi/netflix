...
const GptSearchBar = () => {
  ...
  return (
    <div className='pt-[10%] flex justify-center'>
       <form className='w-1/2 bg-black bg-opacity-80 grid grid-cols-12' onSubmit={(e)=>e.preventDefault()}>
        <input ref={searchText} type='text' className='p-4 m-4 col-span-9 text-lg' placeholder= {lang[langKey].placeholder}/>
        <button className='col-span-3 py-2 px-4 m-4 bg-red-700 text-white rounded-md text-lg'onClick={handleGptSearchClick}>
            {lang[langKey].search}
        </button>
       </form>
    </div>
